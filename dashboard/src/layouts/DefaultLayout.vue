...
<template>
  <div
    :class="themeClass"
    class="min-h-screen flex bg-bg text-text transition-colors duration-300"
  >
    <Sidebar
    ...
    <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
      ...
      >
        <div class="flex items-center gap-4 flex-1">
          ...
          <button
            ...
            class="lg:hidden p-2 rounded-lg border border-current/10 hover:bg-current/5 transition-colors"
          ...
          <div
            class="hidden md:flex relative w-full max-w-md items-center group"
          >
            <span
              class="absolute left-3 opacity-40 group-focus-within:opacity-100 transition-opacity"
            >
              🔍
            </span>
        ...
        <nav class="flex gap-4 items-center ml-4">
          <template v-if="authStore.isAuthenticated">
            <div class="hidden sm:flex flex-col items-end">
              <span
                class="text-[10px] opacity-50 font-black uppercase tracking-widest leading-none mb-1"
                >User</span
              >
            ...
            <span
              v-if="isAdmin"
              class="bg-red-600/10 text-red-500 border border-red-500/30 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-tighter"
            >
              Admin
            </span>
          </template>
          ...
          <button
            @click="toggleTheme"
            class="p-2 rounded-lg border border-current/10 hover:bg-current/5 transition-colors"
            title="Toggle Theme"
          >
            {{ theme === "dark" ? "☀️" : "🌙" }}
          </button>
      ...
      >
        <div
          :class="
            showDashboardFurniture ? 'max-w-7xl mx-auto' : 'w-full h-full'
          "
        >
    ...
    >
      <div
        v-if="showDashboardFurniture && isSidebarOpen"
